...
def find_character(character):
    ...
    if character.lower() == characters.name.lower():
        time.sleep(2)
        return characters
...
def one_vs_one(first_character, second_character):
    ...
    if first_character.fighting > second_character.fighting:
        ...
        char2_total = character_total(
            second_character) + random.randint(0, 6) + random.randint(0, 6)
        if char1_total > char2_total:
            return first_character.name
        if char1_total < char2_total:
            return second_character.name
            ...
            return "It's a draw!"
    # if both characters have equal fighting stat there is a random bonus given to each fighter that is added to their total
    elif first_character.fighting == second_character.fighting:
...
def versus():
    ...
    if mode == '1':
        ...
        winning_team = two_vs_two(
            team1char1, team1char2, team2char1, team2char2)
        print(colors.green + 'The winner is... ' +
              winning_team + '!\n', colors.reset)
        time.sleep(2)
        start()
...
Which option would you like to pick: ''')
    ...
    if choice == '1':
        ...
        if choice.lower() == characters.name.lower():
            Character.print_hero(characters)
            time.sleep(2)
            choice = str(
                input('Would you like to view a photo of this Superhero/Villain (yes / no): '))
            print()
            if choice.lower() in ['y', 'yes', 'sure']:
                Character.show_image(characters)
                time.sleep(1)
                start()
            elif choice.lower() in ['n', 'no', 'nope']:
                time.sleep(1)
                ...
                error_message()
        if choice.lower() != characters.name.lower():
            print(colors.red + 'Superhero/Villain name not found!',
                  colors.reset, '\n')
            time.sleep(2)
            write_to_text_choice = str(input(
...
Which option would you like to pick: ''')
        ...
        if choice == '1':
            webbrowser.open(
                'https://github.com/JordanLeich/Superhero-Index/releases')
            time.sleep(2)
            start()
        elif choice == '2':
            webbrowser.open(
                'https://github.com/JordanLeich/Superhero-Index/graphs/contributors')
            time.sleep(2)
            start()
        elif choice == '3':
            choice = str(
            ...
            for c in characters:
                if choice.lower() == c.name.lower():
                    print(
                        colors.red + 'This hero/villain is already included in the index!\n', colors.reset)
                    time.sleep(1)
                    start()
            request_a_character(choice)
        elif choice == '4':
