...
def find_character(character):
    ...
    if not characters:
        ...
        if write_to_text_choice.lower() in ['y', 'yes', 'sure']:
            ...
            error_message() # TODO reprompt user to fix mistake made
    return characters
...
# recursive if improper input selected in menu to reprompt user
def versus():
...
# will be recursive when an error - so to reprompt user with menu and fix their error
def extras_menu():
